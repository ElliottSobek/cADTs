...
static Node find_prev_node(Node const list, const char *const key) {
	const size_t key_len = strnlen(key, STR_MAX);
...
static bool node_exists(const Node const list, const char *const key) {
	const size_t key_len = strnlen(key, STR_MAX);
	Node cur = list;
...
void s_ll_insert(const S_Ll const list, const char *const key, const char *const value) {
...
void s_ll_insert_sorted(const S_Ll const list, const char *const key, const char *const value) {
	const Node const new_node = create_node(key, value);
...
int s_ll_remove(const S_Ll const list, const char *const key) {
